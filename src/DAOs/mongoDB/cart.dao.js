    ...
    async insertOne(cid, pid, qty) {
        try {
            ...
            if (!product) {
                ...
                const productCart = await Cart.findById(cid);
                ...
                if (product.stock >= qty) {
                    ...
                    await ProductsDao.updateOne(pid, 'stock', product.stock - (oldQty + qty));
    ...
    async deleteAllProducts(cid) {
        try {
            ...
            productCart.products.forEach(async p => {
                const prodMod = await ProductsDao.findById(p.product._id)
